...
function index() {
  ...
  const searchProduct = async (keyword) => {
    if (!keyword) {
      // Si le mot-clé est vide, on remet tous les produits de la catégorie femme
      const response = await fetch(`/api/products?category=femme`);
      const data = await response.json();
      setProducts(data.products);
    } else {
      const normalizedKeyword = normalizeText(keyword);
      // Recherche dans l'API avec le mot-clé avec la catégorie femme
      const response = await fetch(`/api/products?search=${normalizedKeyword}&category=femme`);
      const data = await response.json();
      setProducts(data.products);
    }
  };
