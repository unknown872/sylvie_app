...
export default async function handler(req, res) {
  if (req.method === "POST") {
    ...
    form.parse(req, async (err, fields, files) => {
      ...
      const { firstName, lastName, city, address, phone, email, notes } =
        fields;
      const deliveryQuartier = fields.deliveryQuartier
        ? fields.deliveryQuartier
        : [];
      const deliveryPrice = parseInt(fields.deliveryPrice);
      const totalPrice = parseInt(fields.totalPrice);
      // Récupérer les produits et les images
      const products =
        typeof fields.products === "string"
      ...
      try {
        ...
        // Sauvegarder les images dans le produit
        if (imageFile) {
          // Enregistrer le chemin de l'image principale
          await prisma.product.update({
        ...
        if (otherImageFile) {
          // Enregistrer le chemin de l'image secondaire (si présente)
          await prisma.product.update({
            where: { id: products[0].id },
            data: {
              other_image: `/uploads/${otherImageFile.newFilename}`,
            },
          });
        }
        // Envoi d'un email de confirmation (exemple avec nodemailer)
        ...
        // Configuration de l'email avec les détails de la commande
        // Configuration des emails de confirmation avec Tailwind CSS
...
const mailOptionsAdmin = {
  ...
  `,
        ...
        return res
          .status(201)
          .json({ message: "Commande soumise avec succès", order });
      } catch (error) {
        console.error("Erreur lors de la soumission de la commande:", error);
        return res
          .status(500)
          .json({ error: "Erreur lors de la soumission de la commande." });
      }
    });
  } else {
