...
export default function NewItems({ addToCart, cart, isInWishlist, addToWishlist, wishlist }) {
    ...
    useEffect(() => {
        ...
        if (typeof window !== "undefined") {
            ...
            // Ajouter la définition de l'animation au document si elle n'existe pas déjà
            if (!document.querySelector('#progress-bar-keyframes')) {
    ...
    // Afficher une notification quand un produit est ajouté au panier
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                // Filtrer uniquement les produits nouveaux
                const filteredProducts = data.products.filter(p => isNewProduct(p.createdAt));

                // Simulation d'un délai de chargement
                setTimeout(() => {
                    setProducts(filteredProducts);
                    setIsLoading(false);
                }, 5000); // Délai réduit pour une meilleure expérience utilisateur
            } catch (error) {
    ...
    const isAddToCartDisabled = (product) => {
        const productInCart = cart.find(item => item.id === product.id);
        return productInCart && productInCart.quantity >= product.stock;
    };
    ...
    return (
        <div className="py-10 relative">
            ...
            {isLoading ? (
                ...
                <p className="text-center">Pas de nouveaux produits disponibles!</p>
            ) : (
                <Carousel
